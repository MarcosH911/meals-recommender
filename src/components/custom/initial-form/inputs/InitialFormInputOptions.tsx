      ...
      >
        {options.map((option, index) => (
          <div
            key={index}
            className={twMerge(
              "relative flex w-full items-center gap-2 rounded-lg border border-slate-300 bg-slate-200 px-4 py-2 transition duration-100 hover:border-slate-400 hover:bg-slate-300 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-slate-700 dark:hover:bg-slate-800",
              formValues[name] === option.value &&
                "border-slate-400 bg-slate-300 dark:border-slate-700 dark:bg-slate-800",
            )}
          >
            <label
